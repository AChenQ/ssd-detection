...
def default_prior_box():
    ...
    for k, f in enumerate(feature_map):
        ...
        for i, j in product(range(f), repeat=2):
            ...
            s_k_prime = sqrt(s_k * sk[k + 1] / image_size)
            mean += [cx, cy, s_k_prime, s_k_prime]
            for ar in aspect_ratios[k]:
                mean += [cx, cy, s_k * sqrt(ar), s_k / sqrt(ar)]
                mean += [cx, cy, s_k / sqrt(ar), s_k * sqrt(ar)]
        if use_cuda:
            mean = torch.Tensor(mean).cuda().view(feature_map[k], feature_map[k], -1).contiguous()
        else:
            mean = torch.Tensor(mean).view(feature_map[k], feature_map[k], -1).contiguous()
...
def change_prior_box(box):
    if use_cuda:
        return torch.cat(
            (box[:, :2] - box[:, 2:] / 2, box[:, :2] + box[:, 2:] / 2), 1  # xmin, ymin
        ).cuda()  # xmax, ymax
    else:
        return torch.cat(
            (box[:, :2] - box[:, 2:] / 2, box[:, :2] + box[:, 2:] / 2), 1  # xmin, ymin
...
def point_form(boxes):
    ...
    return torch.cat(
        (boxes[:, :2] - boxes[:, 2:] / 2, boxes[:, :2] + boxes[:, 2:] / 2), 1  # xmin, ymin
    )  # xmax, ymax
