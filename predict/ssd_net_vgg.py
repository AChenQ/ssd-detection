...
class SSD(nn.Module):
    def __init__(self):
        ...
        self.feature_map_conf_1 = nn.Sequential(
            nn.Conv2d(in_channels=512,out_channels=4*class_num,kernel_size=3,stride=1,padding=1)
        )
        self.feature_map_conf_2 = nn.Sequential(
            nn.Conv2d(in_channels=1024,out_channels=6*class_num,kernel_size=3,stride=1,padding=1)
        )
        self.feature_map_conf_3 = nn.Sequential(
            nn.Conv2d(in_channels=512,out_channels=6*class_num,kernel_size=3,stride=1,padding=1)
        )
        self.feature_map_conf_4 = nn.Sequential(
            nn.Conv2d(in_channels=256,out_channels=6*class_num,kernel_size=3,stride=1,padding=1)
        )
        self.feature_map_conf_5 = nn.Sequential(
            nn.Conv2d(in_channels=256,out_channels=4*class_num,kernel_size=3,stride=1,padding=1)
        )
